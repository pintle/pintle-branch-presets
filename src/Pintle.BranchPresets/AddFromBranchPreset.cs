namespace Pintle.BranchPresets
{
	...
	/// that point to other children of the branch be re-pointed to the newly created branch item
		...
		protected virtual void RewriteBranchRenderingDataSources(Item item, BranchItem branchTemplateItem, string branchRoot)
		{
			...
			LayoutHelper.ApplyActionToAllRenderings(item, rendering =>
			{
				...
				if (renderingTargetItem == null)
					Log.Warn("Error while expanding branch template rendering data sources: data source {0} was not resolvable.".FormatWith(rendering.Datasource), this);
